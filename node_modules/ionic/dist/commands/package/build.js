...
let PackageBuildCommand = class PackageBuildCommand extends cli_utils_1.Command {
    preRun(inputs, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sec = new cli_utils_1.SecurityClient(token, this.env.client);
            if (!inputs[0]) {
    ...
    run(inputs, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const token = yield this.env.session.getAppUserToken();
            const deploy = new cli_utils_1.DeployClient(token, this.env.client);
            const sec = new cli_utils_1.SecurityClient(token, this.env.client);
            const pkg = new cli_utils_1.PackageClient(token, this.env.client);
            if (typeof profile === 'string') {
                this.env.tasks.next(`Retrieving security profile ${chalk.bold(profile)}`);
                const p = yield sec.getProfile(profile.toLowerCase());
                this.env.tasks.end();
                if (!p.credentials[platform]) {
                    this.env.log.error(`Profile ${chalk.bold(p.tag)} (${chalk.bold(p.name)}) was found, but didn't have credentials for ${pkg.formatPlatform(platform)}.`);
                    return 1;
                }
            ...
            yield this.env.hooks.fire('command:build', {
                env: this.env,
                inputs,
                options: cli_utils_1.filterOptionsByIntent(this.metadata, options, 'app-scripts'),
            });
            const snapshotRequest = yield upload_1.upload(this.env, { note });
            ...
            const build = yield pkg.queueBuild({
                platform,
...
PackageBuildCommand = __decorate([
        ...
        inputs: [
            {
                ...
                validators: [cli_utils_1.contains([undefined, 'ios', 'android'], {})],
            },
        ],
        options: [
            {
